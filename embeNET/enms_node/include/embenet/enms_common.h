...
// Check structure packing
EMBEUTILS_STATIC_ASSERT(sizeof(ENMS_ServiceInfo) == 18, "sizeof(ENMS_ServiceInfo) should be 18");
...
typedef struct EMBEUTILS_PACKED {
   ...
   * When the number of packets in the packet reception queue reaches the alerting threshold value, the 'alert' condition is signaled. Such a
   * superframe is then counted. The value of queueAlertCnt is the number of such superframes in the last 1000 superframes. It gives information about
   * peak packet queue usage.
   */
  uint16_t queueAlertCnt;
  /**
   * Number of times the packet reception queue was overflown during the last 1000 superframes.
   * When the packet reception queue is overflown it cannot store any more packets and subsequent incoming packets are dropped. When that happens in a
   * given superframe, such a superframe is then counted. The value of queueOverflowCnt is the number of such superframes in the last 1000
   * superframes. It gives information about packet queue failures.
   */
...
// Check structure packing
EMBEUTILS_STATIC_ASSERT(sizeof(ENMS_StatusInfo) == 33, "sizeof(ENMS_StatusInfo) should be 33");
...
// Check structure packing
EMBEUTILS_STATIC_ASSERT(sizeof(ENMS_NeighborInfo) == 10, "sizeof(ENMS_NeighborInfo) should be 10");
