 ...
 *
 * So now:
 * - if we send data to port 2222 using unicast addressing (unicastAddr - from the above example), our node will receive the same data on socket *U*
 * and *A*
 * - if we send data to port 2222 using multicast addressing to group 12 (multicastAddr12 - from the above example), our node will receive the same
 * data on socket *M* and *A*
 * - if we send data to port 2222 using multicast addressing to group 15 (multicastAddr15 - from the above example), our node will receive the same
 * data only on socket *A*
 *
 ...
 * ====================
 *
 * Before using a socket we must register it through a call to @ref EMBENET_UDP_RegisterSocket. This call takes a pointer to a @ref
 * EMBENET_UDP_SocketDescriptor structure that describes the socket. The following example shows how to register a socket that accepts all types of
 * traffic.
 *
 * @code
 ...
 * ============
 *
 * Each socket can be assigned with a user defined pointer called userContext. This variable can point to any user-defined data to you want to bind
 * with the socket. The intended use case is to pass a user context to the data reception handler. Below is an example. Let's assume that we want
 * the reception handler to store the received value in a buffer, that will be set during socket registration:
 *
 * @code
 * char userBuffer[64];
 * @endcode
 ...
 * @note The user MAY register socket with EMBENET_UDP_SOCKET_TYPE_MULTICAST==socketType and group not joined by the node. The user will not receive
 * messages on this socket until the node joines the required group
...
#ifndef EMBENET_UDP_OS_INDEPENDENT
/**
 * @brief Sets the unicast address of the node
 * @param[in] addr pointer to a string containing the IPv6 address, e.g.: "2001:0db8:0000:0000:0000:0000:0000:0001" or "2001:0db8::1"
 * @note the function may be used ONLY in OS dependent implementation of UDP
