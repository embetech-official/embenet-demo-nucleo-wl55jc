...
/**
 * @brief Event handler that is called when the node joins a given network.
 *
...
/**
 * @brief Event handler that is called when the node leaves the network.
 *
...
/**
 * @brief Event handler that is called when the node tries to join a network.
 *
 ...
 */
typedef void (*EMBENET_NODE_OnJoinAttempt)(EMBENET_PANID panId, void const *panData, size_t panDataSize);

/**
 * @brief Event handler that is called when the quick join credentials become obsolete
 *
...
/**
 * @brief Event handler that is called when a UDP datagram was received on an unregistered port.
 *
