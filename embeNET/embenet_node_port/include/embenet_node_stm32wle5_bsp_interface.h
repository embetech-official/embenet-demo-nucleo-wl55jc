 ...
 * @param[in] context optional context that will be passed to the callbacks upon calling
 */
void EMBENET_NODE_BSP_UART_Init(EMBENET_NODE_BSP_UART_Callback_t const txEndCallback, EMBENET_NODE_BSP_UART_Callback_t const rxCallback,
                                void *context);
 ...
 */
void EMBENET_NODE_BSP_UART_WriteByte(uint8_t const c);
 ...
 * @note Eg. if the radio front end has an antenna added with 2dB of gain, then returned offset should be lowered by 2.
 */

int8_t EMBENET_NODE_BSP_GetTxOffset(void);

/**
 * @brief Return RX offset for given RF Path
 * @note Eg. if the radio front end has an antenna added with 2dB of gain, then returned offset should be lowered by 2.
 */
int8_t EMBENET_NODE_BSP_GetRxOffset(void);
...
/** @brief Indicates default value of trimming capacitor in steps of 0.47 pF added to the minimal value:
 *
...
extern EMBENET_NODE_BSP_TCXOVoltage const
    EMBENET_NODE_BSP_TCXOSupplyVoltage;     //< @brief When TCXO is used, and is driven via dedicated pin, selects supply voltage
