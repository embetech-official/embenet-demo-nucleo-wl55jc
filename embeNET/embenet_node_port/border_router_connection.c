...
uint16_t openhdlc_crc(uint16_t crc, uint8_t byte) { return (crc >> 8) ^ fcstab[(crc ^ byte) & 0xff]; }
 ...
 */
static void uartWrite(uint8_t const data);
 ...
 */
static void putByte(uint8_t const byte);
 ...
 */
static void putFlag(uint8_t const flag);
...
size_t EMBENET_BRT_Receive(void *packetBuffer, size_t packetBufferSize) {
  ...
  if(true == frameComplete && inputFrameDataIndex > 0) {
    ...
    crc = (uint16_t)(~crc);
    if((((crc >> 0) & 0xff) == inputFrame[inputFrameDataIndex - 2]) &&
       (((crc >> 8) & 0xff) == inputFrame[inputFrameDataIndex - 1])) { // two last bytes are CRC bytes
      // CRC OK!
...
bool EMBENET_BRT_IsBusy(void) { return isTransmitting; }
...
static void putFlag(uint8_t flag) { uartWrite(flag); }
